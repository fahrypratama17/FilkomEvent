  ...
  <main class="relative flex-1 px-12 py-8">
    <header class="flex items-start justify-between gap-6">
      <div class="mb-8 flex items-center gap-5">
        ...
        <div
          class="relative overflow-hidden shimmer bg-linear-to-r from-secondary-lighter via-white/40 to-white/80 p-4 rounded-4xl backdrop-blur-3xl">
      ...
      <button
        class="flex h-14.5 w-14.5 items-center justify-center rounded-full bg-[#233E98] hover:scale-105 duration-200 shadow-sm">
    ...
    <div class="mb-6 flex items-center justify-between">
      ...
      <a href="{{ route('admin.events.create') }}"
         class="flex items-center gap-2 rounded-2xl bg-primary-lighter px-6 py-3 font-bold text-white shadow-md transition hover:bg-secondary-dark/80 active:scale-95">
    ...
    <section class="mb-9 flex items-end justify-between gap-6">
        ...
        <div>
          ...
          <div class="relative">
            <select id="categoryFilter" name="category_id"
                    class="h-10.5 min-w-63.5 rounded-2xl border border-[#D0D0D0] bg-[#F7F7F7] px-4 pr-18 text-[14px] text-[#2F2F2F] focus:outline-none appearance-none cursor-pointer">
      ...
      <div>
        ...
        <div class="relative">
          <select id="statusFilter" name="event_status"
                  class="h-10.5 min-w-29 rounded-2xl border border-[#D0D0D0] bg-[#F7F7F7] px-4 pr-10 text-[14px] text-[#2F2F2F] focus:outline-none appearance-none cursor-pointer">
            <option value="">Semua Status</option>
            <option value="akan_datang" {{ request('event_status') == 'akan_datang' ? 'selected' : '' }}>Akan Datang
            </option>
            <option value="berlangsung" {{ request('event_status') == 'berlangsung' ? 'selected' : '' }}>Sedang
              Berlangsung
            </option>
            <option value="selesai" {{ request('event_status') == 'selesai' ? 'selected' : '' }}>Selesai</option>
            <option value="dibatalkan" {{ request('event_status') == 'dibatalkan' ? 'selected' : '' }}>Dibatalkan
            </option>
          </select>
        </div>
      </div>
    ...
    <div class="rounded-[20px] border border-gray-200 bg-white p-6 shadow-md">
      <div
        class="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr_1fr] border-b pb-4 text-center text-sm font-bold text-[#1F388B]">
      ...
      <div id="adminEventResults">
        @include('partials.events-management-results', ['events' => $events])
      </div>
    </div>
  </main>
