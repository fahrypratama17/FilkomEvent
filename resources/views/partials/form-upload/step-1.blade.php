<div class="form-step" id="step-1">
  ...
  <div class="grid grid-cols-2 gap-x-15 gap-y-6.5">
    <div>
      ...
      <input type="text" name="title" placeholder="Nama Event" class="admin-input" value="{{ old('title', $event->title ?? '') }}">
    </div>

    <div>
      <label class="admin-label">Mulai Event:</label>
      <div class="relative">
        <input type="text" name="event_start" placeholder="Kapan Event dimulai?" onfocus="this.type='datetime-local'" class="admin-input pr-13.75" value="{{ $eventStartValue }}">
    ...
    <div>
      ...
      <div class="relative">
        <input type="text" name="event_end" placeholder="Kapan Event berakhir?" onfocus="this.type='datetime-local'" class="admin-input pr-13.75" value="{{ $eventEndValue }}">
        <i data-lucide="calendar-days" class="pointer-events-none absolute right-5.75 top-1/2 h-4.5 w-4.5 -translate-y-1/2 text-orange-550"></i>
      </div>
    </div>
    ...
    <div>
      ...
      <input type="text" name="location" placeholder="Lokasi Event" class="admin-input" value="{{ old('location', $event->location ?? '') }}">
    </div>

    <div>
      <label class="admin-label">Kuota:</label>
      <input type="number" name="quota" placeholder="Jumlah peserta" class="admin-input" value="{{ old('quota', $event->quota ?? '') }}">
    </div>
  </div>
</div>
